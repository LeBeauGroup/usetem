...
class MicroscopeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET_V1(self, endpoint, query):
        ...
        elif endpoint == "ColValOpen":
            response = self.server.microscope.get_ColValOpen()
        ...
        elif endpoint == "condenser_stig":
            response = self.server.microscope.get_condenser_stig()
        elif endpoint == "stem_rotation":
            response = self.server.microscope.get_stem_rotation()
    ...
    def do_PUT_V1(self, endpoint, query):
        ...
        if endpoint == "stage_position":
            ...
            pos = dict((k, decoded_content[k]) for k in decoded_content.keys() if k in self.server.microscope.STAGE_AXES)
            self.server.microscope.set_stage_position(pos, method=method)
        elif endpoint == "ColValOpen":
            self.server.microscope.set_ColValOpen(decoded_content)
        elif endpoint == "image_shift":
            ...
            self.server.microscope.set_condenser_stig(decoded_content)
        elif endpoint == "stem_rotation":
            self.server.microscope.set_stem_rotation(decoded_content)
        elif endpoint == "beam_blanked":

...
PyObject* tupleFromVector(ESVision::IVector *vec)
{
    double x, y;
...
bool setVectorFromSequence(ESVision::Vector* vec, PyObject* seq)
{
...
PyObject* tiascriptModule = NULL;

static PyObject* getApplication(void)
{
    ESVision::IApplication* iface;
    HRESULT result = CoCreateInstance(ESVision::Application, NULL, CLSCTX_ALL,
        ESVision::IApplication, (void**)&iface);
    if (FAILED(result)) {
        raiseComError(result);
        return NULL;
    }
...

{
    ...
    // Initialize module
#if PY_MAJOR_VERSION >= 3
    tiascriptModule = PyModule_Create(&moduledef);
#else
    tiascriptModule = Py_InitModule("_tiascript", methods);
#endif
    if (!tiascriptModule)
        return INIT_ERROR;

    // Add exception
    comError = PyErr_NewException("tiascript.COMError", NULL, NULL);
    Py_INCREF(comError);
    PyModule_AddObject(tiascriptModule, "COMError", comError);
    ...
    PyModule_AddObject(tiascriptModule, "Instrument", (PyObject *)&Application_Type);

#if PY_MAJOR_VERSION >= 3
    return tiascriptModule;
#endif
}
